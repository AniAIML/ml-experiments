...
RENDER_SIZE = 96                         # larger canvas used before downscaling
                                         # (room for slant/stretch without clipping)
...
# --------------------------------------------------------------------------------
def get_font_paths():
    # More typefaces = more letter shapes = better generalisation to real
    # handwriting. Comic Sans and Ink Free are included on purpose: they are
    # the closest thing to hand-drawn letters that ships with Windows.
    win = "C:\\Windows\\Fonts\\"
    candidates = [
        ...
        win + "comic.ttf", win + "comicbd.ttf",
        win + "Inkfree.ttf",
        win + "consola.ttf", win + "micross.ttf",
...
# --------------------------------------------------------------------------------
# Uploaded image -> 28x28 white-letter-on-black array, matching training data
# --------------------------------------------------------------------------------
BACKGROUND_OPTIONS = [
    "Auto-detect",
...
def preprocess_upload(img, background=BACKGROUND_OPTIONS[0]):
    arr = np.array(flatten_alpha(img).convert("L"), dtype=np.float32) / 255.0
    if background == BACKGROUND_OPTIONS[0]:
        # The image border is almost always background, so a bright border
    ...
    boosted = Image.fromarray((arr * 255).astype(np.uint8)).filter(ImageFilter.MaxFilter(3))
    arr = np.array(boosted, dtype=np.float32) / 255.0
    return crop_and_center(arr, size=IMG_SIZE)
...
def render_letter(letter, font, angle_range=15, shift_range=4, noise_std=0.03,
                  shear_range=0.3, width_range=0.2):
    ...
    # Random slant (shear) and width squash/stretch about the centre, plus a
    # shift: handwriting leans and varies in width far more than any typeface.
    ...
    a = 1.0 / random.uniform(1.0 - width_range, 1.0 + width_range)
    c = RENDER_SIZE / 2
    img = img.transform(img.size, Image.AFFINE,
                        (a, shear, c - a * c - shear * c + dx, 0, 1, dy),
    ...
    if r < 0.35:
        ...
        thinned = img.filter(ImageFilter.MinFilter(3))   # thinner strokes
        # Light typefaces (e.g. Ink Free) lose whole strokes when thinned,
        # turning a Z into a "/". Only keep the thinned version if at least
        # half the ink survives.
        if np.asarray(thinned).sum() >= 0.5 * np.asarray(img).sum():
            img = thinned
...
class NumpyANN:
    ...
    def train_batch(self, X, y, lr=0.001, beta1=0.9, beta2=0.999, eps=1e-8,
        ...
        for i in range(len(self.weights)):
            ...
            # decays towards zero and, after a few epochs, into the float32
            # denormal range, where CPU arithmetic is many times slower.
            # Flushing those values to exactly 0 changes nothing numerically.
...
if st.session_state.model is None:
    ...
    st.markdown(
        ...
        "- Black letter on white background is fine (Paint's default); "
        "'Auto-detect' below works out which way round your image is."
    )
    ...
    if uploaded_file is not None:
        ...
        final_arr = preprocess_upload(img, background)
        img = flatten_alpha(img).convert("L")
        ...
        st.caption(
            ...
            "resemble your letter, set 'Image colours' above by hand instead "
            "of 'Auto-detect' — that's almost always the cause of wrong predictions."
        )
